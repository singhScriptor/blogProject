...
async function displayBlogs(details) {
  ...
  list.className = "list list-group-item bg-dark d-flex justify-content-between align-items-center text-white";
  ...
  toggleBtn.className = "btn btn-lg btn-danger ms-auto";
  ...
  toggleBtn.addEventListener("click", async () => {
    ...
    detailsDiv.className = "details mt-2 p-2 border rounded bg-dark text-white";
    ...
    submitBtn.addEventListener("click", async () => {
      ...
      if (text) {
        try {
          const res = await axios.post(`http://localhost:3000/blogs/${details.id}/comments`, { text });
